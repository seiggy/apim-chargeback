...
- **APIM Log Truncation**: APIM diagnostic logs cap at 8 KB. The outbound fire-and-forget policy forwards the full payload to the Chargeback API, bypassing that limit.
...
**Container App limit** — default 100 MB. APIM's 8 KB diagnostic log truncation is bypassed because the outbound policy forwards the full response body directly to the Chargeback API's `/api/log` endpoint.
